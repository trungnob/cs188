...
class GameState:
  ...
  def getPacmanPosition( self ):
    return self.data.agentStates[0].configuration.getPosition()
  ...
  def getFood(self):
    ...
    NOTE: calling this function requires a bit of processor
    time, so if you want to check multiple food locations
    in a state, only call getFood once.
    """
    return self.data.food.copy()
  
  def getWalls(self):
    """
    Just like getFood but for walls
    """
    return self.data.layout.walls.copy()
...
def readCommand( argv ):
  ...
  parser.add_option('-d', '--depth', dest='searchDepth', 
                    help=default('the search depth passed to the agent'), default='2')
  parser.add_option('-e', '--eval', dest='evaluationFn', 
                    help=default('the FUNCTION in the pacmanAgents module to use for state evaluation'), 
                    metavar='FUNCTION', default='scoreEvaluation')
  ...
  parser.add_option('-g', '--ghosts', dest='ghost', 
                    ...
                    metavar = 'TYPE', default='RandomGhostAgent')
  ...
  pacmanType = loadAgent(options.pacman, options.nographics)
  ...
  if not options.nographics:
    ...
    graphicsDisplay.GRID_SIZE *= options.zoom
...
def runGames( layout, pacman, ghosts, display, numGames=1 ):
  import __main__
